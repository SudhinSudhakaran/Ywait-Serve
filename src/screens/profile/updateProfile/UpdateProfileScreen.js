...
const UpdateProfileScreen = () => {
  ...
  const createDynamicFields = () => {
    ...
    //     const emailIndex = activeOnlyDynamicList.findIndex(item => item.key.toUpperCase() === 'email'.toUpperCase());
    //     emailIndex !== -1 && activeOnlyDynamicList.splice(emailIndex, 1);
    // }
  ...
  const performProfileUpdate = params => {
    setIsLoading(true);
    DataManager.performProfileUpdate(params).then(
      ([isSuccess, message, data]) => {
        if (isSuccess === true) {
          if (data !== undefined && data !== null) {
            Utilities.showToast(
              t(Translations.SUCCESS),
              Strings.PROFILE_UPDATED_SUCCESS,
              'success',
              'bottom',
            );
            setIsLoading(false);
        ...
        <View
          ...
          <Text
            style={{
              ...
              marginTop:isTablet===true?6:0,
              marginLeft: 20,
              fontSize:isTablet===true?25: 16,
                ...
                return (
                  <View
                    ...
                    <TextInput
                      multiline
                      numberOfLines={1}
                      style={{backgroundColor: Colors.TRANSPARENT ,fontSize:isTablet===true?20:0}}
                      activeUnderlineColor={Colors.PRIMARY_COLOR}
                      label={
                        <Text
                          style={{
                  ...
                  <View
                    ...
                    <View style={{flex: 0.65, marginLeft: 8,}}>
                      <TextInput
                        multiline
                        numberOfLines={1}
                        style={{backgroundColor: Colors.TRANSPARENT,fontSize: isTablet===true?18:0}}
                        activeUnderlineColor={Colors.PRIMARY_COLOR}
                        label={
                          <Text
                            style={{
