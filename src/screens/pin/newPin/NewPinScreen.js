...
const NewPinScreen = () => {
  ...
  const continueButtonAction = (code = enteredPin) => {
    ...
    if (isValidInputs(code) === true) {
      ...
      navigation.push('ConfirmPinScreen', {pin: code});
    }
  };
  const skipButtonAction = (code = enteredPin) => {
    Keyboard.dismiss();
    //SKIP Action
    disablePINAuthForUser();
  };
  ...
  const disablePINAuthForUser = () => {
    ...
    const body = {
      [APIConnections.KEYS.STATUS]: false,
      ...
      ([isSuccess, message, responseData]) => {
        if (isSuccess === true) {
          ...
          if (_verificationData !== undefined && _verificationData !== null) {
            setIsLoading(false);
            if (_verificationData?.enablePinAuthentication === false) {
              //Navigate to dashboard
              existingUserAuthorizationSuccess(_verificationData);
            } else {
              //Navigate to dashboard
              var tempUserDetails = Globals.TEMP_USER_DETAILS;
              tempUserDetails.enablePinAuthentication === false;
              existingUserAuthorizationSuccess(tempUserDetails);
            }
          } else {
            Utilities.showToast(
      ...
      <View
        ...
        <InputScrollView
          ...
          <Text
            ...
            numberOfLines={3}>
            {t(Translations.NEW_PIN_DESCRIPTION)}
          </Text>
