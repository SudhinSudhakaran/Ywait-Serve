...
const ServedListTabScreen = () => {
      ...
      ([isSuccess, message, data]) => {
        if (isSuccess === true) {
          if (data.object !== undefined && data.object !== null) {
            console.log('Serving data', data.object.served);
            setServedList(data.object.served);
            setIsLoading(false);
          } else {
            Utilities.showToast(
  ...
  const renderItem = ({item, index}) => {
    return <ArrivedDataCell item={item} index={index} />;
  };

  const ArrivedDataCell = ({item}) => {
    return isLoading ? (
      <ListLoader />
    ) : (
      <View
        ...
        <View style={{marginBottom: 20, flex: 1}}>
          ...
          <View style={{flexDirection: 'row', marginTop: 10}}>
            <Text
              ...
              {t(Translations.SERVING_STARTED_AT)}
            </Text>
            ...
            <Text
              ...
              {Utilities.getUtcToLocalWithFormat(item?.servingDate, 'hh:mm A')}
            </Text>
          </View>
        </View>

        <View style={{flex: 0.5, paddingTop: 10, marginRight: 10}}>
          <View
            style={{
              borderWidth: 1,
          ...
          <View style={{flexDirection: 'row', marginTop: 10}}>
            ...
            <Text
              ...
              {Utilities.getUtcToLocalWithFormat(item?.arrivingDate, 'hh:mm A')}
            </Text>
          </View>
          <View style={{flexDirection: 'row', marginTop: 10}}>
            <Text
      ...
      <KeyboardAvoidingView
        ...
        <View
          ...
          <FlatList
            ...
            data={isLoading ? dummyNotArrivedListData : servedList}
...
export default React.memo(ServedListTabScreen);
