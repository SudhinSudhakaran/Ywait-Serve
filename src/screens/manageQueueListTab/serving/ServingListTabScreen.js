      ...
      ([isSuccess, message, data]) => {
        if (isSuccess === true) {
          if (data.object !== undefined && data.object !== null) {
            console.log('Serving data', data.object.serving);
            setServingList(data.object.serving);
            setIsLoading(false);
          } else {
            Utilities.showToast(
  ...
  const ArrivedDataCell = ({item}) => {
    return isLoading ? (
      ...
      <View
        ...
        <View style={{flex: 0.5, paddingTop: 10, marginRight: 5}}>
          <View
            style={{
              borderWidth: 1,
      ...
      <KeyboardAvoidingView
        ...
        <View
          ...
          <FlatList
            ...
            data={isLoading ? dummyNotArrivedListData : servingList}
...
export default React.memo(ServingListTabScreen);
