      ...
      ([isSuccess, message, data]) => {
        if (isSuccess === true) {
          if (data.object !== undefined && data.object !== null) {
            console.log(' Arriving data', data.object.arrived);
            setArrivedList(data.object.arrived);
            setIsLoading(false);
          } else {
            Utilities.showToast(
        ...
        <View style={{marginBottom: 20, flex: 1}}>
          ...
          <View style={{marginTop: 8, flexDirection: 'row', marginRight: 10}}>
            ...
            <Text
              ...
              numberOfLines={2}>
              {item?.bookingId ? item?.bookingId : item?.waitlistId}
            </Text>
          </View>
      ...
      <KeyboardAvoidingView
        ...
        <View
          ...
          <FlatList
            ...
            data={isLoading ? dummyNotArrivedListData : arrivedList}
...
export default React.memo(ArrivedListTabScreen);
